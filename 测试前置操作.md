1、配置权限节点：测试网站（不是预发布和正式）数据管理权限节点管理配置->选择一级/二级/其他需求的目录->点击新增->可以上传appicon（图标集合&&各自关系的json文件）文件作为配置文件->权限中文名就是对应模块的名字，权限名是对应的英文名->选择“是”，app对应权限节点显示，选择“否”不显示<br>
点击模块管理->模块管理->新增->新增完保存->对对应模块点击分配权限，搜索到添加好的模块中文->保存
2、企业身份组配置，即把不同的企业放到不同的group中，每个group有不同的权限<br>
3、分配企业模块权限<br>
